...
test(`${testName} ${testTags}`, async () => {
    ...
    Log.info('Mute the SUC');
    await app.open(`${user2.userInfo.firstName} ${user2.userInfo.lastName}`);
    await app.chatController.unMuteConversation();
    await app.messageHubController.clickSideBarChatsButton();

    Log.info(
        `${user2.userInfo.firstName} ${user2.userInfo.lastName} replies back to ${user1.userInfo.firstName} ${user1.userInfo.lastName}`
    );
