...
const user1 = USERS_INTEG1.USER3_CA;
const user2 = USERS_INTEG1.USER2_US;
const user3 = USERS_INTEG1.WAGT3;
...
test(`${testName} ${testTags} @static`, async ({ browser }) => {
    ...
    await test.step(`Step 2 - User1 can open existing conversation by creating a conversation`, async () => {
        await test.step(`WHEN`, async () => {
            await test.step(`User1 creates a conversation with phone numbers that already has an existing conversation `, async () => {
                ...
                await app1.createChatController.fillExternalSearchField(user3.PHONE_NUMBER);
                await app1.createChatController.clickAddPhoneNumber();
        ...
        await test.step(`THEN`, async () => {
            await test.step(`Header elements are displayed`, async () => {
                ...
                await expect(app1.chatController.Pom.AVATAR_SMS_ICON).toBeVisible();
                await expect(app1.chatController.Pom.GROUP_TEXT_NAME).toHaveText('Vega GT Automation');
                await expect(app1.chatController.Pom.GROUP_TEXT_PARTICIPANTS).toHaveText(
                    '+1 343 321 7986, +1 919 578 4041'
                );
            ...
            await test.step(`Footer elements are displayed`, async () => {
                ...
                await expect(app1.chatController.Pom.CHAT_INPUT_DRAFT_EDITOR).toHaveText('Enter a message');
    ...
    await test.step(`Step 3 - User1 can send message`, async () => {
        ...
        await test.step(`THEN`, async () => {
            await test.step(`Message is displayed in chat feed`, async () => {
                await expect(
                    app1.chatController.Pom.MESSAGE_ROW_CONTAINER.nth(-1).locator('.m-auto-message-content')
    ...
    await test.step(`Step 5 - User2 can receive message`, async () => {
        await test.step(`WHEN`, async () => {
            await test.step(`User2 creates a conversation with a phone number that already has an existing conversation `, async () => {
                ...
                await app2.hubHeaderController.selectHeaderMainMenuOption('Text');
                ...
                await app2.createChatController.fillExternalSearchField(user3.PHONE_NUMBER);
                await app2.createChatController.clickAddPhoneNumber();
        ...
        await test.step(`THEN`, async () => {
            await test.step(`Recevied message is displayed in chat feed`, async () => {
                await expect(
                    app2.chatController.Pom.MESSAGE_ROW_CONTAINER.nth(-1).locator('.m-auto-message-content')
                ).toHaveText(randomContentFromUser1);
            });
